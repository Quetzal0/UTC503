...
import requests
import config
from images_request_linux import download_images
...
def history():
    ...
    history_line_array = []
    with open('your_path_here/References/history.txt', 'r') as f:
        for line in f:
            num_in = line.strip()
            history_line_array.append(num_in)
...
def publish():
    ...
    alt_array = []
    with open('your_path_here/References/history.txt', 'a') as f:
        f.write(str(newline))
    with open('your_path_here/References/alt.txt',
    ...
    Connection.api.update_status_with_media(status, image_path)
